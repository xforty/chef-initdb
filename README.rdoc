...
documentation for more info on the options that are optional and have
default values or those that require specific formats.

All actions are contained inside the ((*:initdb*)) hash.

Multiple ((*provider*)) hash's can be specified if you need to connect
...
((*provider*)) using one of these symbols: (({:mysql})),
